...
static int msg_printf(char *buf, int *ctx, int code, char *fmt, ...) {
	...
	va_start(ap, fmt);
	vsnprintf(msg, sizeof(msg), fmt, ap);
	send_server_message("code=%d,msg=%s", code, msg);
	return 0;
}
...
static void ajax_send_message(struct mg_connection *conn,
	...
	if (text[0] != '\0') {
		...
		pthread_rwlock_unlock(&rwlock);
		on_command(text, NULL, &ctx, msg_printf);
		mg_printf(conn, "%s", "true");
	} else {
		mg_printf(conn, "%s", "false");
	}
...
// A handler for the /ajax/send_message endpoint.
static void json_send_message(struct mg_connection *conn,
		const struct mg_request_info *request_info) {
	struct message *message;
	struct session *session;
	char text[sizeof(message->text) - 1];
	...
	if (text[0] != '\0') {
		...
		session = get_session(conn);
		assert(session != NULL);
		on_command(text, message->text, &ctx, json_printf);
		my_strlcpy(message->user, session->user, sizeof(message->user));
		mg_printf(conn, "\n{\n\"ret\":[%s{}]\n}", message->text);
		pthread_rwlock_unlock(&rwlock);
	} else {
...
//	} else
	...
	} else if (strstr(request_info->uri, "gray.jpg")) {
		send_jpg(conn);
	...
	} else if (strcmp(request_info->uri, "/json/send_message") == 0) {
		json_send_message(conn, request_info);
	} else {
...
int webhttp_start(char *root_dir) {

	if (ctx)
		return 0;

	struct mg_callbacks callbacks;
	const char *options[] = { "listening_ports", "8081", "document_root",
			root_dir, NULL };
