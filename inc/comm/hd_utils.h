...
e_uint8* DEV_EXPORT hd_image_stretch(int screen_w, int screen_h, int bpp, e_uint8* src,
		int s_w, int s_h, int s_h_w, int *d_w, int *d_h);

int DEV_EXPORT hd_video_set_src(e_uint8 *src,int s_w,int s_h,float s_h_w);
int DEV_EXPORT hd_video_set_screen(int screen_w,int screen_h,int bpp,void (*set_image)(unsigned char* pixel,int w, int h));
